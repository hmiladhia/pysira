...
#let cvEntry(
  ...
  if metadata.layout.entry.display_tags_first { entryTagListStyle(tags) }
  entryDescriptionStyle(description)
  if not metadata.layout.entry.display_tags_first { entryTagListStyle(tags) }
}
